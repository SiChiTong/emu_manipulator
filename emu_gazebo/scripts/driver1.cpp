              ...
              *objective, b_struct_T options)
{
  ...
  if (solution->state >= 0) {
    ...
    if (maxConstr > 0.002) {
      ...
      solution->maxConstr = b_maxConstraintViolation(&c_workingset,
        solution->xstar);
      ...
      iterate(H, f, solution, memspace, &b_workingset, qrmanager, cholmanager,
              objective, options.MaxIterations, 1.4901161193847657E-10, 0.002);
      if (b_workingset.isActiveConstr->data[(b_workingset.isActiveIdx[3] +
           b_workingset.sizes[3]) - 2]) {
        idx = b_workingset.sizes[0] + b_workingset.sizes[1];
        exitg1 = false;
        while ((!exitg1) && (idx + 1 <= b_workingset.nActiveConstr)) {
          if ((b_workingset.Wid->data[idx] == 4) &&
              (b_workingset.Wlocalidx->data[idx] == b_workingset.sizes[3])) {
            idx_global = idx + 1;
            ...
            i = b_workingset.nVar;
            for (idx = 0; idx < i; idx++) {
              b_workingset.ATwset->data[idx + b_workingset.ATwset->size[0] *
                (idx_global - 1)] = b_workingset.ATwset->data[idx +
      ...
      idxEndIneq = b_workingset.sizes[0] + b_workingset.sizes[1];
      while ((idxStartIneq > idxEndIneq) && (idxStartIneq > c_workingset.nVar))
      {
        ...
        for (idx = 0; idx < i; idx++) {
          b_workingset.ATwset->data[idx + b_workingset.ATwset->size[0] *
            (idxStartIneq - 1)] = b_workingset.ATwset->data[idx +
            b_workingset.ATwset->size[0] * (b_workingset.nActiveConstr - 1)];
        }
