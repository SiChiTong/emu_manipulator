         ...
         b_struct_T qpoptions)
{
  ...
  if (WorkingSet->sizes[1] > 0) {
    ...
    for (k = 0; k <= mEq; k++) {
      WorkingSet->bwset->data[WorkingSet->sizes[0] + k] = WorkingSet->beq->
        data[k];
    }
  }
  ...
  idx_Aineq = 1;
  iy = 13;
  idx_upper = WorkingSet->sizes[3] + 13;
  ...
  for (idx = idxIneqOffset; idx <= mLB; idx++) {
    ...
    switch (WorkingSet->Wid->data[idx - 1]) {
     case 3:
      ...
      idx_Aineq++;
      WorkingSet->bwset->data[idx - 1] = WorkingSet->bineq[ix - 1];
      break;
