...
void addBoundToActiveSetMatrix_(h_struct_T *obj, int TYPE, int idx_local)
{
  ...
  switch (TYPE) {
   case 5:
    ...
    obj->bwset->data[obj->nActiveConstr - 1] = 1.7976931348623157E+308;
    break;

   default:
    idx_bnd_local = obj->indexLB->data[idx_local - 1] - 1;
    obj->bwset->data[obj->nActiveConstr - 1] = obj->lb->data[idx_bnd_local];
    break;
  }
  ...
  obj->ATwset->data[idx_bnd_local + obj->ATwset->size[0] * (obj->nActiveConstr -
    1)] = 2.0 * static_cast<double>((TYPE == 5)) - 1.0;
  idx_bnd_local += 2;
  i1 = obj->nVar;
